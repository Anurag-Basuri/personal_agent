...
export const githubTool = tool(
	async ({ username }) => {
		try {
			...
			if (!eventsRes.ok)
				return `Failed to fetch GitHub events for ${username}. GitHub API Status: ${eventsRes.status}`;

			const eventsText = await eventsRes.text();
			let events;
			try {
				events = JSON.parse(eventsText) as any[];
			} catch (e) {
				return 'GitHub API returned malformed data. Tell the user you cannot fetch events right now.';
			}

			// Fetch user info
			const userRes = await fetch(`https://api.github.com/users/${username}`, { headers, signal: controller.signal });
			clearTimeout(timeout);
			let user = null;
			if (userRes.ok) {
				try {
					user = JSON.parse(await userRes.text()) as any;
				} catch (e) {}
			}
	...
	},
	{
		...
		description: "Fetch Anurag's live GitHub profile stats and recent open-source activity.",
		schema: z.object({
			username: z
				.string()
				.default('anurag-basuri')
				.describe('The GitHub username to fetch. Always defaults to anurag-basuri.'),
		}),
	},
);
