...
export function AuthButton({ className }: { className?: string }) {
    ...
    return (
      <div className={cn("flex items-center gap-4", className)}>
        <div className="flex items-center gap-2">
          {session.user?.image ? (
            <img src={session.user.image} alt="User Avatar" className="h-8 w-8 rounded-full border border-zinc-200" />
          ) : (
  ...
  return (
    <button
      ...
      className={cn(
        "flex h-10 items-center justify-center gap-2 rounded-full bg-zinc-900 px-6 text-sm font-medium text-white shadow-md transition hover:bg-zinc-800 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2",
        className
      )}
    >
